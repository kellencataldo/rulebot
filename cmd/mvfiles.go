...
func iterateFiles(startPage int, prefix string) {
    ...
    fmt.Println("Sanatizing file names")
    ...
    for _, f := range files {
        ...
        _, err := fmt.Sscanf(f.Name(), "PZO2101 %d-%d %s-%d.png", &sp, &ep, &postfix, &filenum)
        //  PZO2101 066-231 Classes-1.png
        if err != nil {
            fmt.Printf("dumb Error scanning file name: %s, error: %s\n", f.Name(), err)
            return
        }

        newName := fmt.Sprintf("%09s.png", strconv.Itoa(filenum))
